...
function compare
{
find $dir -type f -exec md5sum {} \; | sort -k 2 | md5sum | cut -b1-33 > $dir.md5compare 
}
...
#	echo "$dir Backup failed at $(date) check logs" | mailx -s "Backups Failed" $email
	...
	cat $dir.md5compare > $dir.currentmd5
...
{
compare
diff -q $dir.md5compare $dir.currentmd5 > /dev/null 2>> $LOGS
if [ $? -ge 1 ]; then
